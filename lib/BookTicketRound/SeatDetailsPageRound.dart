...
class SeatDetailsPageRound extends StatefulWidget {
  static TripListModel tripListModelReturn = TripListModel();
  const SeatDetailsPageRound({
    Key? key,
    required this.title,
  ...
  @override
  State<SeatDetailsPageRound> createState() => _SeatDetailsPageRound();
}

class _SeatDetailsPageRound extends State<SeatDetailsPageRound> {
  ...
  void initState() {
    ...
    pr.style(message: "Fetching seats");
    pr.show();
    super.initState();
    ...
    GetListData(context)
        .getTripSeats(AppUrl.tripListModelReturn.bus_id,
            widget.from.id, widget.to.id, widget.date)
        .then((value) => {
              setState(() {
                seats = value;
              }),pr.hide()
            });
    fares = AppUrl.tripListModelReturn.fares;
        ...
        return GestureDetector(
          onTap: () {
            ...
            if (selectedSeats.contains(e.seat_id)) {
              ...
              if (AppUrl.passengers.length ==
                  selectedSeats.length) {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                  behavior: SnackBarBehavior.floating,
                  content: Text(
                      'You cant book more than ${AppUrl.passengers.length} seats'),
                ));
                return;
              }
              setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        floatingActionButton:
            selectedSeats.length == AppUrl.passengers.length
                ? Container(
                    height: 80,
                    width: size.height,
                          ...
                          children: [
                            GestureDetector(
                              ...
                              onTap: () async {
                                ...
                                print(
                                    "The current object is ${AppUrl.tripListModelReturn}");
                              },
                            )
                          ],
                        )
