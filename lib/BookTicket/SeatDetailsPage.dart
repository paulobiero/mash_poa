...
class _SeatDetailsPage extends State<SeatDetailsPage> {
  ...
  void initState() {
    ...
    GetListData(context).getTripSeats(BookTicketBasePage.tripListModel.bus_id,widget.from.id,widget.to.id,widget.date).then((value) => {
          setState(() {
            seats = value;
          }),pr.hide()
        });
    fares=BookTicketBasePage.tripListModel.fares;
  ...
  Widget getDoorWidget(double width, double height) {
    return Container(
      ...
      child: Icon(Icons.sensor_door_outlined,color: Colors.white,),
    );
  }
